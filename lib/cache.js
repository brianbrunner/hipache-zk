...
function Cache(config, handlers) {
    ...
    if (this.config.zks) {
        ...
        var hosts = []
        for (var i = 0, len = this.config.zks.length; i < len; i++) {
            var zk = this.config.zks[i]
            hosts.push(zk.host + ":" + zk.port)
        }

        // Configure zooekeeper
    ...
    if (!this.zkClient && !this.redisClient) {
        throw "You must configure either a redis or zookeeper backend." 
    }
    // Only monitor for the active checker if we're using redis
    if (this.redisClient) {
    ...
    // LRU cache for Redis/ZK lookups
...
Cache.prototype.markDeadBackend = function (backendInfo) {
    if (this.redisClient) {
        this._redisMarkDeadBackend(backendInfo)
    }
    if (this.zkClient) {
        this._zkMarkDeadBackend(backendInfo)
    }
...
/*
 * This method marks a dead backend in the redis cache by its backend id
 */
Cache.prototype._redisMarkDeadBackend = function (backendInfo) {
...
/*
 * This method marks a dead backend in the zookeeper cache by its backend id
 */
Cache.prototype._zkMarkDeadBackend = function (backendInfo) {
    var frontendKey = "/dead/" + backendInfo.frontend;
...
Cache.prototype.getBackendFromHostHeader = function (host, callback) {
    ...
    var readFromCache = function (hostKey, cb) {
        ...
        // Don't fire the callback more than once
        var cbOnce = _.once(cb)
        if (this.redisClient) {
            this._redisGetBackendFromHostHeader(host, cbOnce);
        }
        if (this.zkClient) {
            this._zkGetBackendFromHostHeader(host, cbOnce)
        }
    }.bind(this)
...
Cache.prototype._redisGetBackendFromHostHeader = function (hostKey, cb) {
...
Cache.prototype._zkFetchRows = function(hostKey, cb) {
    this.zkClient.a_get_children('/frontend/'+hostKey, false, function(rc, err, children) {
        if (err != null && err != 'ok') {
            ...
            var rows = []
            // process zk data to rows of hostnames
            _.each(children, function(child) {
              rows.push(decodeURIComponent(child))
            })
            cb(null, rows)
        }
    }.bind(this))
...
Cache.prototype._zkGetBackendFromHostHeader = function (hostKey, cb) {
    async.map([hostKey, '*'+this.getDomainName(hostKey), '*'], this._zkFetchRows.bind(this), function(err, rows) {
        this.lru.set(hostKey, rows);
        cb(rows);
    }.bind(this));
};
